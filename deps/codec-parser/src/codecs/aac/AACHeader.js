...
const channelMode = {
  ...
  0b010000000: {
    channels: 2,
    description: "front-left, front-right"
  },
  0b011000000: {
    channels: 3,
  ...
  0b110000000: {
    channels: 6,
    description: "front-center, front-left, front-right, back-left, back-right, LFE-channel"
  },
  0b111000000: {
    channels: 8,
    description: "front-center, front-left, front-right, side-left, side-right, back-left, back-right, LFE-channel"
  }
};
...
class AACHeader extends _CodecHeader.default {
  static *getHeader(codecParser, headerCache, readOffset) {
    ...
    const key = _HeaderCache.default.getKey([data[0], data[1], data[2], data[3] & 111111100 // frame length varies so don't cache it
    ]);
    ...
    if (!cachedHeader) {
      ...
      header.isPrivate = !!(privateBit >> 1); // Byte (3,4 of 7)
      // * `.......H|HH......`: MPEG-4 Channel Configuration (in the case of 0, the channel configuration is sent via an inband PCE)

      header.channelModeBits = new DataView(Uint8Array.of(data[2], data[3]).buffer).getUint16() & 0b111000000;
    ...
    const frameLengthBits = new DataView(Uint8Array.of(0x00, data[3], data[4], data[5]).buffer).getUint32() & 0x3ffe0;
    header.frameLength = frameLengthBits >> 5;
    if (!header.frameLength) return null; // Byte (6,7 of 7)
    // * `...OOOOO|OOOOOO..`: Buffer fullness

    const bufferFullnessBits = new DataView(Uint8Array.of(data[5], data[6]).buffer).getUint16() & 0x1ffc;
    header.bufferFullness = bufferFullnessBits === 0x1ffc ? "VBR" : bufferFullnessBits >> 2; // Byte (7 of 7)
