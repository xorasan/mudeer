...
const channelAssignments = {
  ...
  0b01000000: {
    channels: 5,
    description: "front left, front right, front center, back/surround left, back/surround right"
  },
  0b01010000: {
    channels: 6,
    description: "front left, front right, front center, LFE, back/surround left, back/surround right"
  },
  0b01100000: {
    channels: 7,
    description: "front left, front right, front center, LFE, back center, side left, side right"
  },
  0b01110000: {
    channels: 8,
    description: "front left, front right, front center, LFE, back left, back right, side left, side right"
  },
  0b10000000: {
    channels: 2,
    description: "left/side stereo: channel 0 is the left channel, channel 1 is the side(difference) channel"
  },
  0b10010000: {
    channels: 2,
    description: "right/side stereo: channel 0 is the side(difference) channel, channel 1 is the right channel"
  },
  0b10100000: {
    channels: 2,
    description: "mid/side stereo: channel 0 is the mid(average) channel, channel 1 is the side(difference) channel"
  },
  0b10110000: "reserved",
  0b11000000: "reserved",
  0b11010000: "reserved",
...
class FLACHeader extends _CodecHeader.default {
  static decodeUTF8Int(data) {
    ...
    let next = 2,
        mask = 0xe0,
        value; // determine length of utf-8 character
    ...
    const offset = (next - 1) * 6; // set value for the remaining bits in the length character

    value = data[0] & (mask ^ 0xff) << offset; // set the remaining values
  ...
  static getHeaderFromUint8Array(data, headerCache) {
    const codecParserStub = {
      readRawData: function* (length) {
        if (length > data.length) throw new Error("Out of data while inside an Ogg Page");
        return data;
      }
    };
  ...
  static *getHeader(codecParser, headerCache, readOffset) {
    ...
    header.length += decodedUtf8.next; // Byte (...)
