...
class VorbisHeader extends _CodecHeader.default {
  static getHeaderFromUint8Array(data, headerCache) {
    ...
    const key = _HeaderCache.default.getKey(data.subarray(0, 30));

    const cachedHeader = headerCache.getHeader(key);
    if (cachedHeader) return new VorbisHeader(cachedHeader); // Bytes (1-7 of 30): /01vorbis - Magic Signature

    if (data[0] !== 0x01 || // identification header packet type
    data[1] !== 0x76 || // v
    data[2] !== 0x6f || // o
    data[3] !== 0x72 || // r
    data[4] !== 0x62 || // b
    ...
    header.channels = data[11]; // Byte (13-16 of 30)
    ...
    header.blocksize0 = blockSizes[data[28] & 0b00001111]; // Byte (29 of 30)
