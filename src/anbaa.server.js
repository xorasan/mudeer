...
//					'and updated0 > ? '+
		...
		[ '% '+jawaab.hisaab.uid+':%'/*, jawaab.waqt*/ ]).then(function (qasas) {
			if (qasas.rows.length) {
				...
				mstr = mstr.join(' or ');
				wuqu3aat.query('select * from `'+WUQU3AATNAME+'`.`'+tbl_anb+'` '+
							'where ('+mstr+') '+
							'and updated0 > ? '+
							'order by updated0 asc limit 0,'+limit,
...
shabakah.rafa3('XPO.anbaa', 'XPO.soorah', function (jawaab) {
	...
	helpers.getallbyuid(WUQU3AATNAME, tbl_qasas, [{uid0:qadr}], function (rows) {
		if (rows.length) {
...
//					Files.set.folder('qss');
			...
			if (zaad.length)
			helpers.set(WUQU3AATNAME, tbl_anb, zaad, function (outcome) {
				var nataaij = [];
...
//	$.log( 'XPO.anbaa', '', jawaab.qadr );
	...
	helpers.getallbyuid(WUQU3AATNAME, tbl_qasas, qasas, function (rows) {
		if (rows.length) {
			var zaad = [], pops = [], txrt = {};
