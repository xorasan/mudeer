...
//			$.log.s( 'e$' );
		...
		['intercession', 'get', 'sync', 'upload'].forEach(async function (channel) {
			...
			if (hook_ids.length) {
				...
				for (let hook_id of hook_ids) {
					...
					if ( response[channel] && response[channel][name] && !isundef(response[channel][name][need]) ) {
	...
	var broadcast_process = function (payload, intercession) {
		...
		if (debug_network) $.log.w( module_title, 'Broadcast', payload );
		...
		.then(function (res) {
			...
			$.taxeer('networkbroadcast', async function () {
				let objects = await intercession_process({ channel: 'broadcast' });
				broadcast_process({}, objects);
			}, broadcast_delay);
		});
	};
	var broadcast_start = function () {
		$.taxeer('broadcast_start', async function () {
			broadcast_state = 1;
			update_broadcast_internals();
			let objects = await intercession_process({ channel: 'broadcast' });
			broadcast_process({}, objects);
	...
	var fulfill_gets = function (payload, intercession) { // flush pending get requests
		...
		if (debug_network) $.log.w( module_title, 'Get', payload );
	...
	async function fulfill_fetch(arr, intercession) {
	    ...
	    if (res.headers.has('content-type') && res.headers.get('content-type').includes('application/octet-stream')) {
			...
			if (response.error) {
				...
				var out;
				if ( response.get && response.get[name] && !isundef(response.get[name][need]) ) {
	...
	let intercession = {}; // intercession
	let intercession_process = async function ({ channel } = {}) { // promisfy
		let on_resolve, on_error;
		let promise = new Promise(function (resolve, error) {
			on_resolve = resolve;
			on_error = error;
		});
		let j = 0, arr = Object.keys(intercession);
		let objects = { intercession: {} };

		let results = await Hooks.until_many('network-intercept', {
			before: async function ({ name, need }) {
				return { channel };
			}
		});
		for (let hook_id in results) {
			let { name, need, result } = results[hook_id];
			if (!isundef(result)) {
				objects.intercession[ name ] = objects.intercession[ name ] || {};
				objects.intercession[ name ][ need ] = result;
			}
		}
	...
	var sync_process = function (payload, intercession) {
		...
		if (debug_network) error_log('sync', payload);
	...
	Network = network = {
		...
		upload: async function (name, need, value, payload) {
			if (!name) return;
			if (!payload) return;
			...
			let objects = await intercession_process({ channel: 'upload' });
		...
		sync: function (name, need, value) {
			...
			$.taxeer('networksync', async function () {
				let objects = await intercession_process({ channel: 'sync' });
		...
		get: function (name, need, value) { // { name, need, value }
			...
			$.taxeer('networkfulfill', async function () {
				let objects = await intercession_process({ channel: 'get' });
		...
		fetch: async function (name, need, value) { // returns unique promise
			...
			let objects = await intercession_process({ channel: 'fetch' });
			try {
