...
//		_uglyseed: 'aeiouyüöäß',
		...
		_pickings: [],
		/*
		 * converts json back to slang, each child level is represented by a \t
		...
		toslang: function (obj, tabs) {
			...
			for (var i in obj) {
				...
				if (typeof sub === 'object') {
					slang += '\t'.repeat(tabs) + i + '\n';
					slang += ( _mod.toslang( sub, tabs+1 ) );
				} else {
					if (obj instanceof Array) {
						slang += '\t'.repeat(tabs) + sub + '\n';
					} else if (obj instanceof Object) {
						slang += '\t'.repeat(tabs) + filler(i) + ' ' + sub + '\n';
					} else {
						slang += '\t'.repeat(tabs) + filler(i) + ' ' + sub + '\n';
					}
				}
...
//				if (!path.endsWith('.slang')) alldata += '\n+ignore\n';
...
//					sectionname = ++uid;
		 ...
		 * script
		 * 		'[%slang-script-marker-1%]'
		 * so htm.slang just surrounds it with a regular script tag with text
		...
		dropall: function (parsed, options, UGLY) {
			...
			for (var i in _mod._pickings) {
				...
				if (section.type === _mod.types.svg) {
					...
					parsed	= parsed.replace( '<svg>[%slang-svg-marker-'+i+'%]</svg>', _mod.rawsection(section) );
					...
					parsed	= parsed.replace('<style>[%slang-style-marker-'+i+'%]</style>', result.parsed);
					...
					if (options.linkify) {
						rawjs += result.parsed;
						parsed = parsed.replace('<script>[%slang-script-marker-'+i+'%]</script>', function () {
							if (options.zaboon)
						...
						parsed = parsed.replace('[%slang-script-marker-'+i+'%]', function () {
		 ...
		 * +data
		 * 
		 * IMPORTANT there needs to be at least a body tag in main.htm.slang
		 * 			 otherwise none of this works
		 * 
		 * @param	String		entrypoint	.slang filepath to parse (don't pass in the actual file data)
		...
		multi: function (entrypoint, cboroptions, options) {
			...
			if (options.silent !== true)
				Cli.echo(
					' ~yellow~ ^bright^', options.production ? 'pro' : 'dev',
					' ~~',
					' #', options.buildnumber,
					' ~~ ',
					options.appname
...
//					$.log( options.map );
					...
					var parsedslang	= _mod.parse( text ),
						sections	= _mod.splitter( parsedslang ),
						lastsection	= false;
					...
					Files.set.file('xpo.slang', _mod.toslang( options.map ) );
